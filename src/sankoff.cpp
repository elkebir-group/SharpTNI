...
Sankoff::Sankoff(const BaseTree& T, const int rootLabel = -1)
    :  _T(T)
...
void Sankoff::runConsensus(Node u)
{
    const Digraph& G = tree();
    const int nrInfectedHosts = _T.getNrHost();
...
void Sankoff::computeSol(Node u, int label)
{
    const Digraph& G = tree();
    const int nrInfectedHosts = _T.getNrHost();
...
void Sankoff::computeConsensus(Node u, int label)
{
    const Digraph& G = tree();
    const int nrInfectedHosts = _T.getNrHost();
...
void Sankoff::getSample(Node u, int label)
{
    const Digraph& G = tree();
    const int nrInfectedHosts = _T.getNrHost();
...
void Sankoff::computeWeight(Node u)
{
    const Digraph& G = tree();
    const int nrInfectedHosts = _T.getNrHost();
...
bool Sankoff::enumerateSankoff(std::ostream& psol)
{
    Node root = _T.root();
    const int nrInfectedHosts = _T.getNrHost();
    run(root);
...
void Sankoff::writeSol(std::ostream& psol, const IntStringPairMatrix& sol)
{
...
IntStringPairMatrix Sankoff::enumerateSol(Node u, int label)
{
    ...
    for (Digraph::OutArcIt a(G,u); a != lemon::INVALID; ++a)
    {
        ...
        for (int t = 0; t < nrInfectedHosts; ++t)
        {
            ...
            if (cost_vt == minimum_cost)
            {
                ...
                /*
                for (int solId = 0; solId < _N[v][t]; ++solId)
                {
                    if (childId == 0)
                    {
                        chartChild1.push_back(childTemp[solId]);
                    }
                    else
                    {
                        chartChild2.push_back(childTemp[solId]);
                    }
                }
                */
            }
...
bool Sankoff::next(const UInt64Vector& sizes, UInt64Vector& indices)
{
...
bool Sankoff::solve(std::ostream &psol, bool enumerate, uint64_t enumLimit)
{
...
bool Sankoff::solve(std::ostream &psol)
{
